  ...
  UserIdentity.login = function(
    ...
    userIdentityModel.findOne(
      ...
      function(err, identity) {
        ...
        function defaultFindOrCreate(userObj, cb) {
          var query;
          ...
          userModel.findOrCreate({where: query}, userObj, cb);
        };

        function findOrCreate(userObj, cb) {
          if (!options.findOrCreateUser) {
            return defaultFindOrCreate(userObj, cb);
          }
          // wrapper function to allow fallback to default behavior
          // when both `err` and `user` are empty in the user-defined findOrCreate callback
          options.findOrCreateUser(userObj, (err, user) => {
            if (err || user) cb(err, user);
            else defaultFindOrCreate(userObj, cb);
          });
        };

        findOrCreate(userObj, (err, user) => {
          if (err) {
            return cb(err);
          }
          var date = new Date();
